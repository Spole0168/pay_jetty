...
public class DataDictServiceImpl extends BaseService implements IDataDictService {
	...
	public List<DataDict> list(String type) {
		...
		String cacheKey = _type + CorConstants.CACHE_KEY_SEPERATOR
				+ local.toString();
	...
	public List<DataDict> list(String type, String language) {
		...
		String _type = (null == type) ? null : type.trim();
		String cacheKey = _type + CorConstants.CACHE_KEY_SEPERATOR + language;
		return (List<DataDict>) getDataDictCached(cacheKey);
	...
	/*public ICache getDataDictCache() {
		return CacheManager.getInstance().getCache(CorConstants.DATA_DICT_CACHE);
	}*/
...
//		}else{
			...
			String [] keys = String.valueOf(key).trim().split(CorConstants.CACHE_KEY_SEPERATOR);
			if(keys.length<2){
