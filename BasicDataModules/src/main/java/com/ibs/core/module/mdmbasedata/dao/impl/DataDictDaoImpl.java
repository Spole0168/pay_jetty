...
public class DataDictDaoImpl extends BaseEntityDao<DataDict> implements
	...
	public List<DataDict> list(String type, String language) {
		...
		if (!StringUtils.isEmpty(language)) {
			hql.append(" from DataDict where type = ? and status='").append(
					CorConstants.DATA_DICT__IS_VALID).append("' and  languageCode='")
					.append(language).append("' order by displayOrder");
		} else {
			hql.append(" from DataDict where type = ? and status='").append(
					CorConstants.DATA_DICT__IS_VALID).append("' order by displayOrder");
		}
	...
	public void delete(String id) {
		...
		dataDict.setVersionNo(System.currentTimeMillis());
		dataDict.setStatus(CorConstants.DATA_DICT__IS_NOT_VALID);
		super.saveOrUpdate(dataDict);
	}
	...
	// String type = CorConstants.DICT_BUSINESS_ITEM_TYPE;
	...
	public String getCodeName(String type, String code, String language) {
		...
		hql.append(" from DataDict where type = ? and status='").append(
				CorConstants.DATA_DICT__IS_VALID).append("' and  languageCode='").append(
	...
	@SuppressWarnings("unchecked")
	@Override
	public String findIntfNameByIntfCode(String intfCode) {
		String sql = "select t.NAME from T_MDM_DICT t where t.CODE = ? and t.TYPE = 'CNL_INTF_NUM'";
		SQLQuery sqlQuery = getSession().createSQLQuery(sql);
		List<String> list = sqlQuery.setParameter(0, intfCode).list();
