				...
				try {
					if (oldCache != null) {
						...
						for (Map.Entry<String, Object> entry : maps.entrySet()) {
							// URLEncoder.encode是按HTML4标准转码的，空格会被转换为+号，我们需要的是按RFC-3986方式转码，空格需转换为%20
							maps2.put(entry.getKey(), URLEncoder.encode(entry.getValue().toString(), "UTF-8").replaceAll("\\+", "%20"));
							searchFields.put(entry.getKey(), URLEncoder.encode(entry.getValue().toString(), "UTF-8").replaceAll("\\+", "%20"));
						}
