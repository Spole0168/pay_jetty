 ...
 * CHANGELOG
 *
 * 2010/12/28 修改CacheManager以支持跨节点、跨集群、跨应用的cache同步；
 ...
 * <package name="json-rpc" extends="json-default"> <interceptors>
 * <interceptor name="security" class=
 * "com.ibs.portal.framework.server.interceptor.SecurityInterceptor"/>
 * </interceptors> <action name="refreshCache" class=
 * "com.ibs.portal.framework.server.cache.CacheManager" method="smd">
 * <interceptor-ref name="json"> <param name="enableSMD">true</param>
 * </interceptor-ref> <interceptor-ref name="security"></interceptor-ref>
 * <result type="json"> <param name="enableSMD">true</param> </result> </action>
 * </package>
 * 
 * 
 * CacheManager的初始化的bean定义为(在${Modules}/build/resources/${phase}/deployContext.xml中定义)：
 * <bean id="cacheManager" class=
 * "com.ibs.portal.framework.server.cache.CacheManager">
 * <property name="jsonRpcServiceMethodName" value="refreshSelf"></property>
 * <property name="cacheManagerPeers"> <list>
 * <value>http://192.168.2.202:9094/mdm/refreshCache.action</value>
 * <value>http://192.168.2.202:9095/mdm/refreshCache.action</value> </list>
 * </property> </bean> 每个节点的定义必须指定绝对URL，端口号也必须是WAS实例的监听端口；
 * 
 * 
 * 2011/05/13 针对多server、多模块环境下，节点的膨胀带来的刷新缓存时间过长的问题， 修改为针对每个缓存，配置对应的关注节点列表，比如：
 * <property name="endpoints"> <map> <entry key="mdm" value=
 * "http://localhost:9080/mdm/refreshCache.action"></entry>
 * <entry key="mot" value=
 * "http://localhost:9080/mot/refreshCache.action"></entry> </map> </property>
 * <property name="cacheManagerPeers"> <map>
 * <entry key="_data_dict_cache_region"> <!-- 缓存名称 --> <list> <!-- 关注此缓存的节点名称
 * --> <value>mdm</value> <value>mot</value> </list> </entry> </map> </property>
 *
 ...
 * 这里使用构造值注入的方式来保证两者是同一个对象； 配置如下：
 * 
 * <bean id="cacheManager" class=
 * "com.ibs.portal.framework.server.cache.CacheManager" factory-method=
 * "createInstance"> <constructor-arg> <ref local="cacheManageConfiguration"/>
 * </constructor-arg> </bean> 具体的做法请参考createInstance()的实现；
 * 
 * @author
 *
 */
public class CacheManager {
    ...
    public void refreshAsync(String name, List<String> keys) {
        ...
        try {
            ...
            sql.append(
                    "INSERT INTO T_PBL_CACHE_MESSAGE (MESSAGE_ID, CACHE_NAME, MESSAGE_STATUS, MESSAGE_TIME, CACHE_KEYS) values (?, ?, 0, sysdate, ?)");
        ...
        if (cache != null) {
            for (String key : property.getCacheKeys()) {
                if (cache.containsKey(key)) {
                    ...
                    // If one cache is refreshed, set the refreshed flag to
                    // TRUE;
    ...
    // List peers =
    // this.getConfiguration().getCacheManagerPeers().get(property.getCacheName());
    // if ( CollectionUtils.isNotEmpty(peers) ) {
    ...
    // }
    // } else {
    // logger.error("Could not find cache refresh peers for: " +
    // property.getCacheName());
    // }
    // }
    ...
    // configuration.getJsonRpcServiceMethodName());
    ...
    // CacheRefreshProperty result = gson.fromJson(responseObject.get("result"),
    // CacheRefreshProperty.class);
    //
    // logger.debug("Deserialized result: " + result.getCacheName() + ": " +
    // result.getCacheKeys());
    // } catch (Exception e) {
    // e.printStackTrace();
    // logger.error("Exception happens when request sync cache: " +
    // e.getMessage());
    // } finally {
