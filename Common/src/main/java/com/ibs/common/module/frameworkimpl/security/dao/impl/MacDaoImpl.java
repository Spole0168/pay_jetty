...
public class MacDaoImpl extends BaseEntityDao<Mac> implements IMacDao{
	...
	public Page<MacDto> findMacByPage(QueryPage queryPage, Map<String, String> searchFields) {
		...
		sqlBuffer.append("  from T_PBL_MAC a where 1 = 1 ");
	...
	public Page<MacDto> findMacMenuByPage(QueryPage queryPage,
		...
		sqlBuffer.append("  from T_PBL_MAC a, (select * from T_PBL_MAC_MENU where MENU_ID = ?) b where A.MAC_ID = B.MAC_ID(+) ");
	...
	public Page<MacDto> findMacUserByPage(QueryPage queryPage,
		...
		sqlBuffer.append("select distinct a.MAC_ID as id,a.MAC as mac,a.CPU_ID as cpuId,a.REMARK as remark,STATUS as status, a.RECENTLY_USED_TIME as recentlyUsedTime, (select count(MAC_ID) from T_PBL_MAC_USER where MAC_ID = a.MAC_ID) as userCount ");
		sqlBuffer.append("  from T_PBL_MAC a, T_PBL_MAC_USER b where a.MAC_ID=b.MAC_ID(+)");
	...
	public List<Mac> getMacIdsByMenuId(String menuId) {
		...
		String sql = "select A.MAC_ID as id, A.MAC as mac, A.CPU_ID as cpuId from T_PBL_MAC A, T_PBL_MAC_MENU B where A.MAC_ID = B.MAC_ID and B.MENU_ID = ? ";
	...
	public List<MacInfoDto> getMacInfosByMenuId(String menuId) {
		...
		sql.append(" from T_pbl_mac_menu A, T_pbl_mac B, T_pbl_mac_user C");
