...
public class UserDao extends BaseEntityDao<User> implements IUserDao {
	...
	public void saveInfoForIp(User user){
		...
		sql.append("insert into T_mdm_employee_message (id,create_time,operate_type,password,user_code,user_name,user_org_code,user_org_status,user_status,send_times) ");
		sql.append("values (?,?,?,?,?,?,?,?,?,?)");
...
//			args.add("%" + code.toUpperCase() + "%");
	...
	public int unassignRoleFromUser(String userId, String roleId) {
		String sql = "delete from T_mdm_user_role where user_id=? and role_id=?";
		List<Object> args = new ArrayList<Object>(0);
		args.add(userId);
		args.add(roleId);
	...
	public void insertUser(User user, List<String> defaultRoleCodes) {
		...
		try {
			...
			userSql
					.append("INSERT INTO T_MDM_USER (USER_ID, USER_NAME, USER_PWD, DEFAULT_LOCALE, STATUS,CREATE_TM,CREATOR,CREATE_ID, LAST_UPDATE_TM,LAST_UPDATOR,LAST_UPDATE_ID,USER_TYPE,CREATE_CODE,LAST_UPDATE_CODE,VERSION_NO,DESCRIPTION)"
							+ " values (?, ?, ?,?,?, sysdate, ?,?,?,?,?,?,?,?,?,?)");
