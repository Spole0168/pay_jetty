...
public class ExcelServiceImpl implements IExcelService {
     ...
     * @throws Exception 方法内的异常有ServiceException
     */
    public void exportToFile(Collection result, ExportSetting setting) {
        ...
        try {
            ...
            if (logger.isDebugEnabled())
                logger.debug(fileName + "不存在, 请检查application.properties中的tmp.file.folder配置项, 保证在系统中存在相应的目录.");
            throw new ServiceException(fileName + "不存在, 请检查application.properties中的tmp.file.folder配置项, 保证在系统中存在相应的目录.",
                    e);
        }
    ...
    public void export(OutputStream os, Collection result, ExportSetting setting) {
        ...
        try {
            ...
            // 设置excel标题字体
            wfont = new WritableFont(WritableFont.ARIAL, 16, WritableFont.BOLD, false,
            ...
            // 如果有标题的话，要设置一下偏移
            int offset = 2;
            if (setting.getTitle() == null || setting.getTitle().trim().equals(""))
                offset = 0;
            else {
            ...
            // 设置正文字体
            wfont = new WritableFont(WritableFont.TIMES, 14, WritableFont.BOLD, false,
                    jxl.format.UnderlineStyle.NO_UNDERLINE, jxl.format.Colour.BLACK);
            wcfFC = new WritableCellFormat(wfont);
            // 設置字段類型用於設置
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    if (propertye == null || "".equals(propertye)) {
                        ...
                        if (model.getEditoptions() != null) { // map 转换
                            ...
                            if ("date".equalsIgnoreCase(model.getFormatter())
                                    && StringUtils.isNotEmpty(model.getFormatoptions().getNewformat())) { // format
                                                                                                          // date
                                String format = model.getFormatoptions().getNewformat().replaceAll("Y", "yyyy")
                                        .replaceAll("m", "MM").replaceAll("d", "dd").replaceAll("G", "HH")
                                        .replaceAll("i", "mm").replaceAll("s", "ss").replaceAll("H", "HH");
                            ...
                            } else if ("currency".equalsIgnoreCase(model.getFormatter())) {
                                // 小数点
                                String decimalSeparator = model.getFormatoptions().getDecimalSeparator();
                                // 小数位数(若小数位没有设置,默认为两位小数)
                                Integer decimalPlaces = model.getFormatoptions().getDecimalPlaces();
                                ...
                                BigDecimal currency = new BigDecimal(propertye);
                                double recurrency = currency.setScale(decimalPlaces, decimalPlaces).doubleValue();
                                // 金额格式
                                String currencyFormat = getCurrencyFomat(decimalSeparator, decimalPlaces);
                                NumberFormat nformat = new NumberFormat(currencyFormat);
                                WritableCellFormat wcfN = new WritableCellFormat(nformat); // 设置表单格式
                                writableCell = new Number(i, rowIndex, recurrency, wcfN);
                        ...
                        if (exportColType == null) {
                            writableCell = new Label(i, rowIndex, BeanUtils.getProperty(obj, col));
                        } else {
            ...
            if (logger.isDebugEnabled())
                logger.debug("Excel行数超过65536行限制！");
            throw new ServiceException("Excel行数超过65536行限制！", e);
        } catch (Exception e) {
            if (logger.isDebugEnabled())
                logger.debug("Excel文件操作失败！");
     ...
     * @return
     */
    private static String getCurrencyFomat(String decimalSeparator, Integer decimalPlaces) {
