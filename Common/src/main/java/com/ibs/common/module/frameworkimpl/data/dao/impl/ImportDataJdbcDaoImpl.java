...
public class ImportDataJdbcDaoImpl extends BaseTransformJdbcDao implements IImportDataDao {
	...
	public void batchSaveImportData(final List<ImportData> datas) {
		...
		String sql = "INSERT /*+append*/ INTO T_PBL_IMPORT_DATA (ID, IMPORT_ID, IMPORT_TIME, ROW_NO, IS_SUCCESS, ERROR_MESSAGE, EXCEL_ROW_ID, EXCEL_ROW_CODE, EXCEL_DATA,SUM1,SUM2,SUM3,SUM4,SUM5) values (?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?)";
...
//				ps.setNull(6, Types.VARCHAR);
	...
	public IPage<ImportData> findImportDataByPage(QueryPage queryPage,
		...
		sql.append(" FROM T_PBL_IMPORT_DATA o WHERE o.IMPORT_ID = ?").append(PARTITION_LIMIT);
...
//		queryPage.addPageFooterColumn(new PageFooterColumn("rowNo", "COUNT(*)"));
	...
	public List<ImportData> findImportDataList(String importId) {
		...
		sql.append(" FROM T_PBL_IMPORT_DATA o WHERE o.IS_SUCCESS = 1 and o.IMPORT_ID = ?").append(PARTITION_LIMIT);
	...
	public boolean haveImportErrData(String importId) {
		...
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT IMPORT_ID FROM T_PBL_IMPORT_DATA o WHERE o.IS_SUCCESS = 0 and o.IMPORT_ID = ? and rownum = 1 ").append(PARTITION_LIMIT);
