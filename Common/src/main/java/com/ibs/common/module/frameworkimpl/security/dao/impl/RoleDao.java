...
//		return super.findPage(hql.toString(), args, pageSize, pageIndex);
	...
	public IPage<RoleDto> getRolePageBySql(
		...
		sql.append("select distinct r.role_id as id," +
				...
				" from T_mdm_role r" +
				" left join T_mdm_app a on r.app_id=a.app_id");
/*		if (null != groupIds && groupIds.size()>0){
			sql.append(" left join T_mdm_role_group_role grp on r.role_id=grp.role_id");
		}*/
		if(null != userId && !"".equals(userId)){
			sql.append(" left join T_mdm_user_role ur on r.role_id=ur.role_id and user_id=?");
			args.add(userId);
		}
...
//				sql2.append(" and r.role_id not in (select role_id from T_mdm_user_role where user_id=?)");
...
//				sql.append("select * from (").append(sql1).append(" union ").append(sql2).append(")");
				...
				sql.append(" and r.role_id not in (select role_id from T_mdm_user_role where user_id=?)");
				args.add(userId);
			}
		}
	...
	public int grantManageMenuToRole(final String roleId, final String menuId) {
		String sql = "insert into T_mdm_role_man_menu(role_id, menu_id) values(?,?)";
		List<Object> args = new ArrayList<Object>();
		args.add(roleId);
		args.add(menuId);
	...
	public int unGrantManageMenuToRole(final String roleId, final String menuId) {
		String sql = "delete from T_mdm_role_man_menu where role_id=? and menu_id=?";
		List<Object> args = new ArrayList<Object>();
		args.add(roleId);
		args.add(menuId);
		return super.executeSql(sql, args);
