...
<script type="text/javascript" charset="UTF-8">
	var addUrl 		= "merchantInfoChgAdd.action";
	var listUrl 		= "";
	var searchUrl 		= "";
	...
	function doSearch(){
		...
		$("#gridTable").jqGrid("setGridParam",{url: "merchantChgSearch.action" }).trigger("reloadGrid");
	}
	...
	$().ready(function() {
		...
		$("#updateBtn").click(function(){
			...
			if(status !="03"&&status !="04"){
				showWarning("只有审核失败和变更申请的记录可以进行修改！");
		    	return false;
			}
			if(id!=null){
		...
		//	 alert("提示信息="+status);
			if(id == null ){
				showWarning("请选择一条记录进行删除！");
		    	return false;
			}
			if(status !="04"){
				showWarning("请选择提交变更申请的记录进行删除！");
		    	return false;
			}
		...
		$("#gridTable").gridUtil().simpleGrid({
			...
			pager: "#gridPager",
			toolbar: [true,"top"],
			caption: "${app:i18n('merchant.merchantChgInfoListJsp.tableTitle')}",
	...
	<div class="layout">
		<div class="block m-b">
			<div class="block_title">
				<h3>${app:i18n('merchant.merchantChgInfoListJsp.Title')}</h3>
			</div>
			...
			<div class="block_container">
				...
				<div class="block">
					...
					<div id="gridToolbar">
					<app:isPermission resource='CORE_MERT_CHG_UPDATE'>
						...
						<app:isPermission resource='CORE_MERT_CHG_REVIEW'>
						...
						<app:isPermission resource='CORE_MERT_CHG_DELETE'>
						<a id="delBtn" class="l-btn-plain l-btn m-l4"><span
							class="l-btn-left"><span class="l-btn-text icon-view">${app:i18n('global.jsp.delete')}</span></span></a>
					</app:isPermission>
					</div>
				</div>
			</div>
