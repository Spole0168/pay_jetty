		                  ...
		                  +"&dataDict.value="+encodeURI($("#dataDict_value").val())
		                   +"&dataDict.extendAtt1="+encodeURI($("#extendAtt1").val())
		                    +"&dataDict.displayOrder="+encodeURI($("#displayOrder").val())
		                     +"&dataDict.description="+encodeURI($("#description").val())
		                      +"&isModify="+encodeURI($("#isModify").val()) 
// qiyongping add start
							+"&dataDict.ctgCode="+encodeURI($("#ctgCode").val()) 
// qiyongping add end
...
<thead>
	...
	<tr>
	 <th  >显示顺序：</th>
		...
		<td><input type="text" name="dataDict.displayOrder" id="displayOrder" class="digits width_b"
			size="25" maxlength="9" value="${dataDict.displayOrder}" /></td>
<!-- qiyongping 20170717 add start  -->
	<th  >CTG_CODE：</th>
		 
		<td><input type="text" name="dataDict.ctgCode" id="ctgCode" class="width_b"
			size="25" maxlength="100" value="${dataDict.ctgCode}" /></td>
<!-- qiyongping 20170717 add end -->		
	</tr>
	...
	<tr>
		...
		<th  > 描述：</th>
								 ...
								 <td colspan="3"  ><s:textarea      id="description" cols="68"  rows="2"  name="dataDict.description" > </s:textarea>
	...
	<td colspan="7">
	 <s:if test="dataDict.status=='01'|| dataDict.id==''||dataDict.id==null  ">
