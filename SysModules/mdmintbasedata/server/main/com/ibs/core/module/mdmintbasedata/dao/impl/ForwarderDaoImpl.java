...
public class ForwarderDaoImpl extends BaseEntityDao<Forwarder> implements
	...
	final static String checkData = "select lpad((substr(nvl(max(CODE),'IA999999000000'),9,6)+1),6,'0') "
			+ "as code FROM core.t_MDM_FORWARDER  where  ORG_CODE=?";
	...
	public  List<ForwarderSimple> findUniversal(
		...
		sql
				.append("select ID as id ,CODE as code,NAME as name,ORG_CODE as orgCode  "
						+ " from core.t_MDM_FORWARDER where 1=1 ");
	...
	public IPage<Forwarder> findForwarderNotInGroup(String operateOrgCode,
		...
		StringBuffer sql = new StringBuffer();
		sql
				.append("select  o.id as id,o.code as code ,o.name as name  from core.t_mdm_forwarder o left join  "
						+ " (select distinct ogo.forwarder_id fid from core.t_mdm_forwarder_group_forwarde ogo "
						+ "left join core.t_mdm_forwarder_group og on ogo.forwarder_group_id=og.id  where "
